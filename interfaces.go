...
// UnboundedSegmentProvider is an abstraction for querying user membership in unbounded segments.
// The caller provides an implementation of this interface to NewEvaluatorWithUnboundedSegments.
type UnboundedSegmentProvider interface {
	// GetUserMembership queries a snapshot of the current segment state for a specific user.
	//
	// The underlying unbounded segment store implementation will use a hash of the user key, rather
	// than the raw key. But computing the hash is the responsibility of the UnboundedSegmentProvider
	...
	//
	// If the returned UnboundedSegmentMembership is nil, it is treated the same as an implementation
	// whose IsUserIncluded and IsUserExcluded methods always return false.
	GetUserMembership(
		userKey string,
	) (UnboundedSegmentMembership, ldreason.UnboundedSegmentsStatus)
}

// UnboundedSegmentMembership is the return type of UnboundedSegmentProvider.GetUserMembership(). It
