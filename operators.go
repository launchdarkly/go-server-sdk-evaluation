...
const (
	...
	// OperatorMatches matches a user value and clause value if they are both strings and the latter is a valid regular
	// expression that matches the former.
	...
	//
	// A valid timestamp is either a string in RFC3339/ISO8601 format, or a number which is treated as Unix milliseconds.
	...
	//
	// A semantic version is a string that either follows the Semantic Versions 2.0 spec, or is an abbreviated version
	// consisting of digits and optional periods in the form "m" (equivalent to m.0.0) or "m.n" (equivalent to m.n.0).
